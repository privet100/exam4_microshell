...
int	putstr_fd2(char *s1, char *s2)
{
	while (*s1)
		write(2, s1++, 1);
	while(s2 && *s2)
...
int execve_fd(char *argv[], int i, int tmp_fd, char *env[])
{
	dup2(tmp_fd, STDIN_FILENO);
	close(tmp_fd);
	argv[i] = NULL; 	// overwrite ; or | or NULL with NULL -> input for execve, no impact in the parent process
	execve(argv[0], argv, env);
	return (putstr_fd2("error: cannot execute ", argv[0]));
}
...
int	main(int argc, char *argv[], char *env[])
{
	...
	while (argv[i] && argv[i + 1]) //check the end
	{
		...
		if (strcmp(argv[0], "cd") == 0 && i != 2)
			putstr_fd2("error: cd: bad arguments", NULL);
		else if (strcmp(argv[0], "cd") == 0 && i == 2 && chdir(argv[1]) != 0)
			putstr_fd2("error: cd: cannot change directory to ", argv[1]);
		else if(i > 0 && argv[i] != NULL && strcmp(argv[i], "|") == 0)
		{
			...
			if ( fork() == 0)
			{
				...
				close(fd[0]);               //
				close(fd[1]);               //
				if (execve_fd(argv, i, tmp_fd, env))
					return (1);
			}
			else
			{
